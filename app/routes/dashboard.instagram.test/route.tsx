...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    return redirect('/dashboard/instagram/test', { headers });
  } catch (error) {
    const code = error instanceof IntegrationFailure && errorMessages[error.code] ? error.code : 'database_unavailable';
    return redirect(`/dashboard/instagram/test?error=${code}`, { headers });
  }
}
  ...
  return (
    <main className="ig-page">
      ...
      <div className="ig-content">
        <p className="ig-eyebrow"><a href="/dashboard/integrations/instagram">Instagram Publishing</a> / API test</p>
        <h1>API test archive</h1>
